...
async function handlePublish() {
  ...
  try {
    ...
    request.post('/api/v1/goods/listing/', { goods_id: currentGoods.value.id, platforms: editForm.targetPlatforms, price: calculatedPrice.value })
      .then(() => {
        currentGoods.value._published = true
        ElMessage.success('上货成功！')
        previewVisible.value = false
      })
      .catch(err => ElMessage.error(err?.message || '上货失败'))
...
async function handlePublishBatch() {
  ...
  try {
    ...
    request.post('/api/v1/goods/listing/batch/', { goods_ids: goodsIds, platforms: editForm.targetPlatforms })
      .then(() => {
        items.forEach(item => { item._published = true })
        ElMessage.success(`批量上货成功！共 ${items.length} 件`)
      })
      .catch(err => ElMessage.error(err?.message || '批量上货失败'))
      .finally(() => { publishLoading.value = false })
  } catch {
    // 用户取消
  } finally {
